-- Equilibrium v2.7.2 | single file, no external modules required
...
-- Load Notification: Clean "v2.7.2 • Universal Hub" (no verbose hex codes)
...
do
    local AboutData={
        Version="2.7.2",
        Build="10K-RP",
        SpecialThanks={"Verity team","Fleece Utility","SchizHub","Base_Rework","DarkHub","TakkuHub","Special thanks to contributors"},
        FavoriteGames={
            {name="Ninja Legends", creator="Scriptbloxian Studios", placeId=3956818381, joinEnabled=true, serverHopEnabled=true},
            {name="Phantom Forces", creator="StyLiS Studios", placeId=292439477, joinEnabled=true, serverHopEnabled=true},
            {name="Arsenal", creator="ROLVe Community", placeId=286090429, joinEnabled=true, serverHopEnabled=true},
            {name="Jailbreak", creator="Badimo", placeId=606849621, joinEnabled=true, serverHopEnabled=true},
            {name="BedWars", creator="Easy.gg", placeId=8737899170, joinEnabled=true, serverHopEnabled=true},
            {name="Adopt Me!", creator="DreamCraft", placeId=920587237, joinEnabled=true, serverHopEnabled=true},
            {name="Universal", description="Teleport / teleport-tested reference", placeId=nil, joinEnabled=false, serverHopEnabled=false, status="Reference only"},
        },
    ...
    for i, preset in ipairs(AssetPresetRegistry) do
        ...
        b.Activated:Connect(function()
            if not valid then setInsertionState("Failed"); pushToast("Invalid preset ID "..pid,"warn",1) return end
            setInsertionState("Inserting")
            local ok, err = pcall(function() previewImg.Image="rbxassetid://"..pid end)
            local target = previewImg:GetFullName()
            if ok and previewImg.Image~="" then
                lastInsertionRecord={Preset=preset.Name, AssetId=pid, PreviewTarget=target, Target=target, InsertParent=previewImg.Parent and previewImg.Parent:GetFullName() or "Workspace", State="Inserted", InsertedAt=os.clock()}
                setInsertionState("Inserted")
                pushToast(preset.Name.." inserted","warn",1)
            else
                lastInsertionRecord={Preset=preset.Name, AssetId=pid, PreviewTarget=target, Target=target, InsertParent=previewImg.Parent and previewImg.Parent:GetFullName() or "Workspace", State="Failed", InsertedAt=os.clock()}
                setInsertionState("Failed")
    ...
    insertBtn.Activated:Connect(function()
        ...
        if id=="" then setInsertionState("Failed"); pushToast("Enter numeric ID","warn",1) return end
        if #id<6 then setInsertionState("Failed"); pushToast("Invalid asset ID (too short)","warn",1) return end
        setInsertionState("Inserting")
        ...
        if not pcall(function() return game:GetService("InsertService") end) then
            ...
            pushToast("Unavailable in this environment","warn",1)
            return
        end
        if ok then
            lastInsertionRecord={Preset="Custom", AssetId=id, PreviewTarget=target, Target=target, InsertParent=previewImg.Parent and previewImg.Parent:GetFullName() or "Workspace", State="Inserted", InsertedAt=os.clock()}
            setInsertionState("Inserted")
            pushToast("Inserted rbxassetid://"..id,"warn",1)
        else
            lastInsertionRecord={Preset="Custom", AssetId=id, PreviewTarget=target, Target=target, InsertParent=previewImg.Parent and previewImg.Parent:GetFullName() or "Workspace", State="Failed", InsertedAt=os.clock()}
            setInsertionState("Failed")
            pushToast("Failed: "..tostring(err),"warn",1)
        end
    end)
...
task.delay(0.12,function()
    ...
    pushToast("Equilibrium v2.7.2 • Universal Hub", "warn", 2.5)
