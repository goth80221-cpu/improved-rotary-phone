...
do
    ...
    local function isCooldown(name) local last=ReactionResolver._cooldowns[name] or 0 return tick() - last < 2 end
    function ReactionResolver.handle(event)
        if not event then return end
        -- resolver consumes specific events only, not generic "interaction"
        if event.type=="interaction" then return end
        if not isRelevant(event) then return end
        if isCooldown("Excited") then return end
        -- conversation check: if ConversationActive, still allow Excited for 7B proof (no block)
        local effect = REACTION_EFFECTS.Excited
        if not effect then return end
        ReactionResolver._cooldowns["Excited"] = tick()
        if effect.emotion then Verity.SetEmotion(effect.emotion) end
        if effect.reaction then Verity.PlayReaction(effect.reaction) end
        if effect.wardrobe and effect.wardrobe.enable then
            for _,id in ipairs(effect.wardrobe.enable) do WardrobeVisual.enable(id) end
        end
    end
