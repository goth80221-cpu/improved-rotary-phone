...
-- PASS 8: AssistantState — boring context only (ReactionResolver decides behavior)
local AssistantState = { ConversationActive=false, CurrentTopic=nil, LastMessage=nil }
...
do
    local function isCooldown(name, sec) local last=BehaviorResolver._cooldowns[name] or 0 return tick() - last < (sec or 1.5) end
    function BehaviorResolver.Resolve(event, assistantState, personality, mood)
        ...
        if isCooldown("BehaviorAny", 1.0) then
