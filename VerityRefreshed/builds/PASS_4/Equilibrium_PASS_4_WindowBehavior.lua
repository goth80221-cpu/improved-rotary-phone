...
local HubState = { Draggable=true, Locked=false }
...
do local drag=false; local sd,sp
    ...
    titleBar.InputBegan:Connect(function(i) if isMax or minimized or (not HubState.Draggable) then return end if i.UserInputType==Enum.UserInputType.MouseButton1 then drag=true sd=i.Position sp=Vector2.new(shell.Position.X.Offset,shell.Position.Y.Offset) end end)
