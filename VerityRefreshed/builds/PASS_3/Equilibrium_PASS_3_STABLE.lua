...
local function ApplyTheme(themeId, save)
    local th = GetTheme(themeId)
    -- Custom: if Theme_07 and CustomColors exist, use them
    if themeId=="Theme_07" and hubAppearance.CustomColors then
        ...
        if next(custom) then th={id="Theme_07", name="Custom", colors=setmetatable(custom, {__index=th.colors}), effects=th.effects} end
    end
    local oldT={}
    for k,v in pairs(T) do oldT[k]=v end
    -- update semantic tokens (Hub only)
    for k,v in pairs(th.colors) do T[k]=v end
    T.titleBar = Color3.fromHex("0f0f0f")
    T.line = T.border
    T.accent2 = th.colors.accent
    hubAppearance.Theme = th.id
    -- refresh cached Hub UI (registry)
    for _,e in ipairs(themedRegistry) do
        if e.obj and e.obj.Parent then
            local c = T[e.token] or th.colors[e.token]
            if c then pcall(function() e.obj[e.prop]=c end) end
        end
    end
    -- whole-Hub walk via HubRoot ThemeRole or fallback by old color (Hub only, Verity outside HubRoot untouched)
    pcall(function()
        if HubRoot then
            for _,inst in ipairs(HubRoot:GetDescendants()) do
                ...
                if role and T[role] then
                    ...
                    -- fallback for elements created before ThemeRole (Slate baseline)
    ...
    EVENTS.fire("theme.changed", th.id)
end
local function RefreshTheme() ApplyTheme(hubAppearance.Theme, false) end
...
do
    ...
    for _,tid in ipairs(THEME_ORDER) do
        ...
        b.Activated:Connect(function()
            ...
            ApplyTheme(tid, true) refreshThemeButtons() pushToast("Theme: "..th.name,"warn",1.4)
        end)
        themeBtns[tid]=b
        -- mini preview dot
